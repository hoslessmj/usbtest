...
public:
    ...
    bool    hwrite(uchar a1,uchar a2, uchar a3,uchar a4,uchar a5);


    int ftdi_usb_close();
    int ftdi_to_clkbits_AM(int baudrate,unsigned long* encoded_divisor);

    int ftdi_set_interface(enum ftdi_interface  interface);

    int ftdi_setflowctrl(int    flowctrl);
    int ftdi_usb_purge_tx_buffer();
    int ftdi_usb_purge_rx_buffer();
    int ftdi_set_latency_timer(unsigned char latency);
    int ftdi_set_bitmode(unsigned char btmask,unsigned char mode);
    int ftdi_read_data(unsigned char*   buf,int len);
    int ftdi_poll_modem_status(unsigned short*  status);
    int ftdi_write_data(unsigned char*  buf,int size);
    ...
    int ftdi_usb_open();
    void ftdi_usb_open_internal();
    int ftdi_usb_reset();
    int ftdi_set_baudrate(int   baudrate);

    int ftdi_convert_baudrate(int br,unsigned short* value,unsigned short* index);

    int ftdi_to_clkbits(int baudrate,unsigned int clk,int clk_div,unsigned long* encoded_divisor);


    void    ftdi_stream_restart(libusb_transfer_cb_fn   cb,int packcount);
    void    ftdi_stream_start(libusb_transfer_cb_fn     callback,int    pack_count);

    ftdi_context*        m_ftdi;
    husb*               m_usb;
public:
    pthread_cond_t      m_cond;
    pthread_cond_t      m_cond_all_finish;
